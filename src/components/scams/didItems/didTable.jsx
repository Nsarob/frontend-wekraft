...
export default function DidTables() {
  ...
  const handleDeleted = async (itemsId) => {
    try {
        ...
        const response = await fetch(`https://wekraft-c156ff639ea6.herokuapp.com/API/didyou/delete/${itemsId}`, {
            method: 'DELETE',
            headers: {
  ...
  useEffect(()=>{
    const fetchDatas = async () =>{
     try {
      const token = localStorage.getItem('token')
       const response = await axios.get(`https://wekraft-c156ff639ea6.herokuapp.com/API/didyou/get`,{
     headers: {
      'auth-token':token,
      'Content-Type': 'application/json',
     }
